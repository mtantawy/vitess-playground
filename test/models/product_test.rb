...
class ProductTest < ActiveSupport::TestCase
  ...
  test "Product.create retries on ActiveRecord::RecordNotUnique Error" do
    # mock Product.generate_sku which uses Faker::Alphanumeric.alphanumeric
    # make it return same sku twice, then different sku on 3rd try
    Product.stubs(:generate_sku).returns("SteamDeck", "SteamDeck", "SteamDeck2")
  ...
  test "Product.create calls generate_* methods" do
    Product.expects(:generate_sku).once
    Product.expects(:generate_name).once
    Product.expects(:generate_description).once
    Product.expects(:generate_quantity).once
    Product.create
  end
end
